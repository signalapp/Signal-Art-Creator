...
const LazyReactSortable = React.lazy(() => import('./ReactSortable'));
...
export function ArtGrid({ mode, showGuide }: Props): JSX.Element {
  ...
  const handleDrop = React.useCallback<DropZoneProps['onDrop']>(
    async files => {
      dispatch(initializeImages(files.map(({ path }) => path)));
      await Promise.all(
  ...
  const fallback = <div className={styles.grid}>{frames}</div>;
